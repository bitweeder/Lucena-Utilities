/*------------------------------------------------------------------------------
	...
	This is a gently hacked-up copy of filesystem/operations.cpp taken from the
...
#include "lulFilesystemCommon.hpp"
	...
	#include <linux/version.h>
		...
		#define LUL_USE_SENDFILE
	#endif
#elif defined (__APPLE__) || __has_include (<copyfile.h>)
	#include <copyfile.h>
	#define LUL_USE_COPYFILE
#endif

#if !defined (__APPLE__)
	#define LUL_USE_CLOCK_GETTIME
#endif

#if !defined (CLOCK_REALTIME) || !defined (LUL_USE_CLOCK_GETTIME)
	#include <sys/time.h>
#endif
...
using string_view_pair = std::pair<string_view_t, string_view_t>;
...
private:
  void makeState(ParserState NewState, PosPtr Start, PosPtr End) noexcept {
    State = NewState;
    //	Assume End > Start, End and Start non-null, etc
    RawEntry = string_view_t(Start, static_cast <size_t> (End - Start));
  }
...
struct FileDescriptor {
  ...
  template <class... Args>
  static FileDescriptor create(const path* p, std::error_code& ec, Args... args) {
    ec.clear();
    int fd;
...
bool posix_ftruncate(const FileDescriptor& fd, size_t to_size, std::error_code& ec) {
  // SEEME Assumes to_size will convert cleanly
  if (::ftruncate(fd.fd, static_cast <off_t> (to_size)) == -1) {
    ec = capture_errno();
    return true;
  }
  ec.clear();
...
#if defined(LUL_USE_CLOCK_GETTIME) && defined(CLOCK_REALTIME)
...
#else
  typedef std::chrono::duration<rep, std::micro> __microsecs;
  timeval tv;
  gettimeofday(&tv, 0);
  return time_point(__secs(tv.tv_sec) + __microsecs(tv.tv_usec));
#endif // LUL_USE_CLOCK_GETTIME && CLOCK_REALTIME
}
...
// SEEME This is disabled as it’s only needed on Windows (possibly), but it
// doesn’t work correctly without fstream support. Note, though, that MSVS has
// a perfectly acceptable <filesystem> implementation, and users of other
// Standard Libraries are probably using something less gimpy than Apple LLVM’s
// stunted implementation.
__attribute__((unused)) bool copy_file_impl_default([[maybe_unused]] FileDescriptor& read_fd,
                                                    [[maybe_unused]] FileDescriptor& write_fd,
                                                    std::error_code& ec) {
//  std::ifstream in;
...
bool copy_file_impl(FileDescriptor& from, FileDescriptor& to, std::error_code& ec) {
#if defined(LUL_USE_SENDFILE)
...
bool __copy_file(const path& from, const path& to, copy_options options,
  ...
  if (!is_regular_file(from_st)) {
    if (not m_ec)
      m_ec = std::make_error_code(std::errc::not_supported);
    return err.report(m_ec);
  }
...
bool __create_directory(path const& p, path const& attributes, std::error_code* ec) {
...
path __current_path(std::error_code* ec) {
  ...
  auto size = static_cast <size_t> (::pathconf(".", _PC_PATH_MAX));
  _LIBCPP_ASSERT(size >= 0, "pathconf returned a 0 as max size");

  auto buff = std::unique_ptr<char[]>(new char[size + 1]);
  char* ret;
  if ((ret = ::getcwd(buff.get(), size)) == nullptr)
...
uintmax_t __file_size(const path& p, std::error_code* ec) {
...
uintmax_t __hard_link_count(const path& p, std::error_code* ec) {
...
void __last_write_time(const path& p, file_time_type new_time, std::error_code* ec) {
  using detail::fs_time;
...
uintmax_t remove_all_impl(path const& p, std::error_code& ec) {
...
uintmax_t __remove_all(const path& p, std::error_code* ec) {
...
void __resize_file(const path& p, uintmax_t size, std::error_code* ec) {
...
path __temp_directory_path(std::error_code* ec) {
  ...
  if (!exists(st) || !is_directory(st))
    return err.report(std::errc::not_a_directory, "path \"%s\" is not a directory",
                      p);
...
size_t hash_value(const path& __p) noexcept {
  ...
  while (PP) {
    hash_value = details::__hash_combine(hash_value, hasher(*PP));
    ++PP;
  }
  return hash_value;
}
