/*------------------------------------------------------------------------------
	...
	This is a gently hacked-up copy of filesystem/directory_iterator.cpp taken
	from the libc++ 7 filesystem implementation, and used under the University
	of Illinois/NCSA Open Source License. See “license/libc++ License” for
	details.
...
namespace stdproxy::filesystem { namespace detail { namespace {
...
static file_type get_file_type([[maybe_unused]] DirEntT* ent, long) {
  return file_type::none;
}

static std::pair<std::string_view, file_type> posix_readdir(DIR* dir_stream,
                                                  std::error_code& ec) {
...
public:
  ...
  __dir_stream(const path& root, directory_options opts, std::error_code& ec)
...
directory_iterator::directory_iterator(const path& p, std::error_code* ec,
  ...
  std::error_code m_ec;
  __imp_ = std::make_shared<__dir_stream>(p, opts, m_ec);
  if (ec)
    *ec = m_ec;
  if (!__imp_->good()) {
    __imp_.reset();
...
struct recursive_directory_iterator::__shared_imp {
  std::stack<__dir_stream> __stack_;
...
recursive_directory_iterator::recursive_directory_iterator(
  ...
  __imp_ = std::make_shared<__shared_imp>();
...
int recursive_directory_iterator::depth() const {
  return static_cast <int> (__imp_->__stack_.size()) - 1;
}
...
void recursive_directory_iterator::__advance(std::error_code* ec) {
