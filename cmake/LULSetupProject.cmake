#[[#############################################################################
	...
	This file is modified from QuickCppLibSetupProject.cmake in
	...
	details. Mostly, we just strip stuff out that we have no need of to reduce
	the maintenance surface and avoid implying bunch of functionality we don’t
	support. Additionally, the file is renamed to avoid conflicts and it sits
...
include(LULParseLibrarySources)
  ...
  if(CLANG_TIDY_EXECUTABLE MATCHES "CLANG_TIDY_EXECUTABLE-NOTFOUND")
    ...
    if(NOT TARGET ${PROJECT_NAME}_lint)
      add_custom_target(${PROJECT_NAME}_lint
        ...
        COMMAND "${PYTHON_EXECUTABLE}" "${CTEST_LUL_SCRIPTS}/run-clang-tidy.py" -clang-tidy-binary="${CLANG_TIDY_EXECUTABLE}" -target-filter="${PROJECT_NAME}_hl--" -header-filter=.* -export-fixes=${PROJECT_NAME}_fixes/fixes.yaml
    ...
    if(NOT TARGET ${PROJECT_NAME}_lint-fix)
      add_custom_target(${PROJECT_NAME}_lint-fix
        "${PYTHON_EXECUTABLE}" "${CTEST_LUL_SCRIPTS}/run-clang-tidy.py" -clang-tidy-binary="${CLANG_TIDY_EXECUTABLE}" -target-filter="${PROJECT_NAME}_hl--"  -header-filter=.* -fix -format
